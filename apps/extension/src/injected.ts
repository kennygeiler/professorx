/**
 * Injected script -- runs in the PAGE context (not content script isolated world).
 * This can intercept the real window.fetch that Twitter uses.
 * Sends captured tweet data back to the content script via window.postMessage.
 */
...
function parseTweetResult(result: Record<string, unknown>): ParsedTweet | null {
  if (result.__typename === 'TweetWithVisibilityResults' && result.tweet) {
    result = result.tweet as Record<string, unknown>;
  }
  if (result.__typename !== 'Tweet' && result.__typename !== undefined) return null;
  ...
  const metrics: ParsedTweet['metrics'] = {
      ...
      cloned.json().then((data: unknown) => {
        ...
        for (const entry of entries) {
          ...
          if (tweetResult && typeof tweetResult === 'object') {
            const parsed = parseTweetResult(tweetResult as Record<string, unknown>);
...
console.log('[ProfessorX] Fetch interceptor installed in page context');
